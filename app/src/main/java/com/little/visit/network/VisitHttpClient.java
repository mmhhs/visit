package com.little.visit.network;
...
/**
 * 网络访问三方库实例 HttpClient
 */
public class VisitHttpClient extends LittleHttpClient {
    private static int REQUEST_TIMEOUT = 60*1000;//设置请求超时时间60秒钟
    private static int DATA_TIMEOUT = 120*1000;  //设置等待数据超时时间120秒钟
    private static VisitHttpClient visitHttpClient = null;

    public VisitHttpClient() {
    }

    public static synchronized VisitHttpClient getInstance()
    {
        if (visitHttpClient == null)
            visitHttpClient = new VisitHttpClient();
        return visitHttpClient;
    }
     ...
     * @param isDebug
     */
    public void setLogPrint(boolean isDebug){
    ...
    public static HttpClient getTrustAllClient() {
        ...
        HttpConnectionParams.setSoTimeout(params, DATA_TIMEOUT); //设置请求超时
